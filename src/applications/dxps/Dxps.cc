...
//    if( timer ) startTimer( timer );
    ...
    for( map<OverlayKey, NfPair>::iterator cit = it->second.getChildrenBegin();
      ...
      if (subscriptionfilter)
      {
        ...
        ismatched=matchit(cit->second.second,filter->getFilter());
      }
    ...
    if( it->second.getSubscription() ) {
      if (subscriptionfilter){
        Filter* filter = dynamic_cast<Filter*>(dataMsg->getEncapsulatedPacket()->getEncapsulatedPacket());
        ismatched=matchit(it->second.getOwnFilterList(),filter->getFilter());
      }
...
bool  Dxps::matchit(FilterList fromTable, OverlayKey pubKey)
{
...
bool Dxps::keycompare(OverlayKey subKey,OverlayKey pubKey){
    //if (subKey.ZERO
    ...
    //return true;
    OverlayKey tmp;
        if ((pubKey & subKey ^ subKey)==OverlayKey::ZERO) return true;
    return false;
  }
