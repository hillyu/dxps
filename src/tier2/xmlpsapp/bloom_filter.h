...
public:
 ...
 //bloom_filter()
       ...
       //bit_table_ = new cell_type[table_size_ / bits_per_char];
       //std::fill_n(bit_table_,(table_size_ / bits_per_char),0x00);
       ////std::cout<<"initialize bloom_filter with default constructor\n";
   //}
 bloom_filter(cell_type* btable, size_t bsize)
	   : bit_table_(0),
	        predicted_element_count_(10),
	        inserted_element_count_(0),
	        random_seed_(0xA5A5A5A5),
       ...
       table_size_ = bsize;
      bit_table_ = new cell_type[table_size_ / bits_per_char];
      std::copy(btable,btable+ (table_size_ / bits_per_char),bit_table_);
      //std::cout<<"copy bit table complete\n";
          } 
 bloom_filter(const bloom_filter& filter)
   ...
   inline bool operator < (const bloom_filter& filter) const{
     for (std::size_t i = 0; i < (table_size_ / bits_per_char); ++i)
     {
       if (bit_table_[i] > filter.bit_table_[i])
         return false;
       else if(bit_table_[i] < filter.bit_table_[i])
         return true;
     }
     return false;
   ...
   const cell_type* table() const { return bit_table_; }
    inline const bloom_filter operator & (const bloom_filter& a) const 
{
   return bloom_filter(*this) &= a;
}

inline bloom_filter operator | (const bloom_filter& a) const
{
   return bloom_filter(*this) |= a;
}
