...
void XmlPsApp::handleTimerEvent( cMessage* msg )
{
  if( msg == timer ) {
    ...
    //if (random < subRate && joinGroups) {
        SubGen sub (bloom_l,bloom_k,overlay->getThisNode().getKey());
...
SubGen::SubGen(int l, int k, OverlayKey mykey){
   //    std::ifstream file (filepath.c_str());
  std::string filepath="/home/hill/sim/OverSim-20101103/samplefiles/xpaths/bib.xpelist";
  std::ifstream file (filepath.c_str());
  std::string line ;
